    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 auto-rows-fr">
        @forelse($favorites as $favorite)
            <div class="favorite-card group h-full">
                <div
                    class="bg-slate-900/80 backdrop-blur-sm rounded-xl shadow-lg border border-slate-800/80 overflow-hidden group
                 hover:shadow-teal-500/10 hover:shadow-2xl hover:border-teal-500/50 transition-all duration-300 transform hover:-translate-y-1 h-[400px] flex flex-col">
                    <div class="relative overflow-hidden h-48 flex-shrink-0">
                        <img src="{{ asset('storage/' . $favorite->event->image) }}" alt="{{ $favorite->event->name }}"
                        ...
                        <span
                            class="absolute top-2 left-2 px-2 py-1 bg-slate-950/90 backdrop-blur-sm text-teal-300 rounded-lg text-xs font-medium border border-teal-500/30">
                    ...
                    <div class="p-6 flex flex-col flex-grow">
                        <div class="flex justify-between items-start mb-3">
                            <h3
                                class="text-lg font-semibold text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-emerald-400 truncate max-w-[70%]">
                                {{ $favorite->event->name }}
                            </h3>
                        </div>
                        ...
                        <div class="space-y-2 mb-4 flex-grow">
                            <p class="text-sm text-gray-400 flex items-center">
                                ...
                                <span
                                    class="truncate">{{ \Carbon\Carbon::parse($favorite->event->event_date)->format('d M Y') }}</span>
                            </p>
                            <p class="text-sm text-gray-400 flex items-center">
                                <svg class="w-4 h-4 mr-2 text-teal-500 flex-shrink-0" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
        ...
        @empty
            ...
            <div
                class="sm:col-span-2 lg:col-span-3 flex flex-col items-center justify-center p-8 bg-gray-800 rounded-lg">
